...
uint8_t USB_Send_Int_Dec(int val, int numbers)
{
	int i, num;
	int factor = 1;
	uint8_t Send_Success = FALSE;
...
uint8_t USB_Send_Int_Hex(int val, int numbers, uint8_t Send_0x)
{
	int i;
	uint8_t Send_Success = FALSE;
	uint8_t Hex_Number;
	...
	for(i=0;i<numbers;i++)
	{
		...
		Send_Success = USB_Send_Char((char)Hex_Number);
